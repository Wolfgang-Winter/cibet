...
public class CibetInterceptorEncryptIT extends AbstractArquillian {

   private static Logger log = Logger.getLogger(CibetInterceptorEncryptIT.class);
   ...
   public static WebArchive createDeployment() {
      String warName = CibetInterceptorEncryptIT.class.getSimpleName() + ".war";
   ...
   @Before
   public void beforeCibetInterceptorEncryptIT() {
      Context.start();
   ...
   public void interceptReleaseWithInfolog() throws Exception {
      ...
      Assert.assertEquals(true, ar.getResource().isEncrypted());
   ...
   public void interceptComplexSignatureArchive() throws Exception {
      ...
      Iterator<ResourceParameter> iter2 = res.getParameters().iterator();
      while (iter2.hasNext()) {
         log.debug(iter2.next());
      }
   ...
   public void statefulEJB() throws Exception {
      ...
      List<Object> list = ejbS.testInvoke2("H�ls");
      Assert.assertNull(list);
   ...
   public void interceptReleaseWithException() throws Exception {
      ...
      List<Object> list = ejb.testInvoke2("H�ls");
      Assert.assertNull(list);
   ...
   public void redoArchive() throws Exception {
      ...
      Assert.assertEquals("H�ls", list2.get(0));
   ...
   public void redo4Eyes() throws Exception {
      ...
      // List rlist2 = (List) ejb.release(l.get(1), "Good Fullmoon");
      Context.sessionScope().setUser(USER);

      // check
      Assert.assertEquals(6, rlist.size());
      Assert.assertEquals(6, rlist.size());
      Assert.assertEquals("H�ls", rlist.get(0));
